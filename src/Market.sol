...
contract Market {
  ...
  event AuctionCreated(address tokenAddress,uint tokenId,address seller,uint minBid,Categories category,uint endDate);
//This implementation is only for 721's thats why it takes into account the token ID
  function createAuction(address _tokenAddress, uint _tokenId,uint _minBid, Categories _category) external {
